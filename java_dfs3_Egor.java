// using Egor Kulikov :: Graph
// https://github.com/EgorKulikov/yaal/blob/master/lib/main/net/egork/graph/Graph.java
...
class Graph {
    ...
    public final int addSimpleEdge(int from, int to) {
    	// System.out.format("addSimpleEdge: %d -> %d\n", from, to);
 ...
 */
public class java_dfs3_Egor implements Runnable {
    public int dfs(Graph g, int v, int p) {
        int depth = 0;
        for (int e = g.firstOutbound(v); e != -1; e = g.nextOutbound(e)) {
            int x = g.destination(e);
            // System.out.format("%d -> %d [e=%d, next=%d]\n", v, x, e, g.nextOutbound(e));
            if (x != p) {
    ...
    public static void main(String[] args) {
        new Thread(null, new java_dfs3_Egor(), "1", 1 << 29).start();
    }
    public void run() {
        try {
            ...
            Graph g = Graph.createGraph(n, from, to);
